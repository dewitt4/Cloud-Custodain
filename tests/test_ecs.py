...
class TestEcsService(BaseTest):
    ...
    def test_ecs_service_resource(self):
        ...
        p = self.load_policy(
            {'name': 'all-ecs',
             'resource': 'ecs-service'}, session_factory=session_factory)
        resources = p.run()
...
class TestEcsTaskDefinition(BaseTest):
    ...
    def test_task_definition_resource(self):
        ...
        for r in resources:
            for c in r['containerDefinitions']:
                images.add(c['image'])
        self.assertEqual(sorted(images), [
            'nginx:latest', 'postgres:latest', 'redis:latest'])
    ...
    def test_task_definition_delete(self):
        ...
        resources = p.run()
        self.assertEqual(len(resources), 1)
        self.assertEqual(resources[0]['containerDefinitions'][0]['image'],
                         'postgres:latest')
...
class TestEcsTask(BaseTest):
    ...
    def test_task_resource(self):
        ...
        p = self.load_policy(
            {'name': 'tasks',
             'resource': 'ecs-task'}, session_factory=session_factory)
        resources = p.run()
    ...
    def test_task_delete(self):
        ...
        tasks = client.list_tasks(
            cluster=resources[0]['clusterArn'])['taskArns']
        self.assertFalse(
            set([r['taskArn'] for r in resources]).intersection(
                tasks))
