...
class MonitoredCloudtrailMetric(Filter):
    ...
    def find_subscribed_topic_arns(self, session, topicArns):
    ...
    def cloudtrail_matches_filter(self, trail):
        ...
        filters = self.fetch_metric_filters_for_log_group(session, groupName)
        ...
        if self.data.get('alarm', True):
            ...
            if self.data.get('topic-subscription'):
                ...
                if not alarmSNSTopics:
                    return False
                consideredSet = self.find_subscribed_topic_arns(session, alarmSNSTopics)
